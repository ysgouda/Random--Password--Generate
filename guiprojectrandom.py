...
#create a class
class GUI():
    def __init__(self,master):#parent window
        self.master = master
        ...
        self.accept = Button(text="ACCEPT", bd=3, relief='solid', padx=1, pady=1, font='forte 15', fg='darkblue', bg='green', command=self.accept_fields)
        ...
        #reset password
        self.reset = Button(text="RESET", bd=3, relief='solid', padx=1, pady=1, font='forte 15', fg='darkblue', bg='green', command=self.reset_fields)
    ...
    def generate_pass(self):
        ...
        if length<6:#password length is atleast 6 charector
            messagebox.showerror("Error", "Password must be atleast 6 characters long")
            self.textfield.configure(text="")#text fill
            return
        else:
            self.blank_label1.configure(text="")#blank lable fill

        self.generated_password_textfield.delete(0, length)

        u = random.randint(1, length-3)#uppercase letter 1, length -3
        l = random.randint(1, length-2-u)#lowercase letter 1, length-2-u
        c = random.randint(1, length-1-u-l)#charecters
        n = length-u-l-c
    ...
    #accept password in to user.db databse
    ...
    def show_fields(self):#then field show
